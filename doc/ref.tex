...
\rhead{\rm\thepage}
\lfoot{Robotics Toolbox Release 8}  %% CHANGE
\rfoot{Peter Corke, December 2008}       %% CHANGE
\cfoot{}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\function{angvec2tr, angvec2r}
\mpur{Convert angle/vector form to a \hom\ or \rot}

\msyn{
T = angvec2tr(theta, v)\\[10pt]
R = angvec2r(theta, v)\\[10pt]
}

\mdes{Returns a \hom\ or \rot\  representing a 
rotation of \var{theta} radians about the vector \var{v}.
For the \hom\ the translational component is set to zero.}

\msa{rotx, roty, rotz, quaternion}
\vfil\eject 
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\function{eul2tr, eul2r}
\mpur{Convert Euler angles to a \hom\ or \rot}

\msyn{
T = eul2tr([$\phi$ $\theta$ $\psi$])\\[0pt]
T = eul2tr($\phi$, $\theta$, $\psi$)\\[10pt]
R = eul2r([$\phi$ $\theta$ $\psi$])\\[0pt]
R = eul2r($\phi$, $\theta$, $\psi$)
}

\mdes{Returns a \hom\ or \rot\ for the specified
Euler angles in radians.
\[
 R_Z(\phi) R_Y(\theta) R_Z(\psi)
\]
For the \hom\ value the translational component is set to zero.
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\function{Fanuc10L}
\mpur{Create a Fanuc 10L robot object}

\msyn{
Fanuc10L
}

\mdes{Creates the \textbf{robot} object \var{R} which describes the kinematic 
characteristics of a AM120iB/10L manipulator.
The kinematic conventions used are as per Paul and Zhang, and
all quantities are in standard SI units.

Also defined is the joint coordinate vector \var{q0}
corresponding to the mastering position.}

\msa{robot, puma560akb, stanford, MotomanHP6, S4ABB2p8}

\mauthor{Wynand Swart, Mega Robots CC, P/O Box 8412, Pretoria, 0001, South Africa,
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\function{ishomog}
\mpur{Test if argument is a \hom}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\function{isrot}
\mpur{Test if argument is a \rot}

\msyn{
isrot(x)
}

\mdes{Returns true if \var{x} is a 3 $\times$ 3 matrix.}
\msa{ishomog, isvec}
\vfil\eject 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\function{isvec}
\mpur{Test if argument is a 3-vector}

\msyn{
isvec(x)
}

\mdes{Returns true if \var{x} is , either a $3 \times 1$ or $1 \times 3$ matrix.}
\msa{ishomog, isrot}
\vfil\eject 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\function{MotomanHP6}
\mpur{Create a Motoman HP 6 robot object}
...
\mdes{Creates the \textbf{robot} object \var{R} which describes the kinematic 
characteristics of a Motoman HP6 manipulator.
The kinematic conventions used are as per Paul and Zhang, and
all quantities are in standard SI units.
...
\msa{robot, puma560akb, stanford, Fanuc10L, S4ABB2p8}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\function{oa2tr, oa2r}
\mpur{Convert OA vectors to \hom\ or \rot}
...
\mdes{Returns a \hom\ or \rot\ specified in
terms of the Cartesian orientation and approach vectors \var{o} and
\var{a} respectively.}

\malg{\[
{\bf R} = \left[ \begin{array}{ccc}
	\vec{\hat{o}} \times \vec{\hat{a}} & \vec{\hat{o}} & \vec{\hat{a}}
	 \end{array} \right]
\]
where $\vec{\hat{o}}$ and $\vec{\hat{a}}$ are unit vectors corresponding
to \var{o} and \var{a} respectively.}

\mcau{An extra cross-product is used to ensure orthonormality.  \var{a} is the only column that is 
guaranteed to be unchanged from that specified in the call. }
...
\msa{robot, puma560akb, stanford, Fanuc10L, MotomanHP6, S4ABB2p8}
\mref{R.~P. Paul and H.~Zhang, ``Computationally efficient kinematics for
  manipulators with spherical wrists,'' {\em Int. J. Robot. Res.}, vol.~5,
    no.~2, pp.~32--44, 1986.
...
\msa{robot, puma560, stanford, Fanuc10L, MotomanHP6, S4ABB2p8}
\mref{
...
\var{q1 + q2} & returns the elementwise sum of quaternion elements \\
\var{q1 - q2} & returns he elementwise sum of quaternion elements \\
...
\mdes{Return a \rot\ representing a 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\function{rpy2tr, rpy2r}
\mpur{Roll/pitch/yaw angles to \hom\ or \rot}
...
the Robotics Toolbox.  It pops up a graphical menu of demos which will run in
the command window.  The demos display tutorial information and require the user
to hit the enter key to display the next pageful of text.

Demos can also be accessed through {\Mlab}'s own demo system, using the \var{demos}
command or the command menu option.  Then the user can navigate to the Robot Toolbox demo
menu.
}

\mcau{This script clears all variables in the workspace and deletes
all figures.  It also adds the \var{demos} directory to your {\Mlab} path on first
invocation.}
\vfil\eject 
...
\mdes{Creates the \textbf{robot} object \var{R} which describes the kinematic 
characteristics of an ABB S4 2.8 manipulator.
The kinematic conventions used are as per Paul and Zhang, and
all quantities are in standard SI units.

Also defined is the joint coordinate vector \var{q0}
corresponding to the synchronization position.}

\msa{robot, puma560akb, stanford, Fanuc10L, MotomanHP6}
...
\msa{robot, puma560, puma560akb, Fanuc10L, MotomanHP6, S4ABB2p8}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\function{tr2angvec}
\mpur{Convert a \hom\ or \rot\ to angle/vector form}

\msyn{
[theta, v] = tr2angvec(T)
}

\mdes{Converts a \hom\ or \rot\ to a 
rotation of \var{theta} radians about the vector \var{v}.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\function{tr2eul}
\mpur{Convert a \hom\ or \rot\  to Euler angles}

\msyn{
e = tr2eul(M)
}

\mdes{Returns a vector of Euler angles [$\phi$, $\theta$, $\psi$], in radians,
corresponding
to  \var{M}.
\var{M} is either a \rot, or the rotation part of the \hom\ is taken.
\[
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\function{tr2rpy}
\mpur{Convert a \hom\ or \rot\ to roll/pitch/yaw angles}

\msyn{
e = tr2rpy(T)
}

\mdes{Returns a vector of roll/pitch/yaw angles, [roll, pitch, yaw], in radians,
corresponding
to  \var{M}.
\var{M} is either a \rot, or the rotation part of the \hom\ is taken.
\[
...
rotation submatrix $\vec{n} = \vec{o} \times \vec{a}$, $\vec{o} = \vec{a} \times \vec{n}$.}
...
\mdes{Displays a 3D plot which shows how the standard axes are transformed
by the transformation \var{T}.}

\mex{Display the coordinate frame represented by a \hom.}
\begin{verbatim}
...
\centerline{\psfig{figure=figs/trplot.eps,width=10cm}}
